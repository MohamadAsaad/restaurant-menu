document.addEventListener('DOMContentLoaded', () => {
    ...
    const ui = {
        ...
        modalCloseBtn: document.getElementById('modal-close-btn'), // <-- تم إضافة هذا السطر لتصحيح الخطأ
    ...
    const api = {
        load: async () => {
            try {
                const response = await fetch('../data/menu.json?t=' + new Date().getTime());
        ...
        save: async () => {
            ...
            try {
                const response = await fetch('save-menu.php', {
                    method: 'POST',
